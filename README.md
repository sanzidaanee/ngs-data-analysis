...
Adapter & Quality Trimming (fastp)
       │
       ▼
Alignment to Reference Genome — chr20/chr21 (BWA-MEM)  →  SAM files
       │
       ▼
SAM → BAM Conversion (SAMtools)
       │
       ▼
Add Read Groups (GATK AddOrReplaceReadGroups)
       │
       ▼
Coordinate Sorting (GATK SortSam)
       │
       ▼
...
(GATK Mutect2)                            (GATK HaplotypeCaller → GVCF)
       │                                              │
       ▼                                              ▼
FilterMutectCalls                         GenomicsDBImport → GenotypeGVCFs
       │                                              │
       ▼                                              ▼
Extract SNPs & Indels (bcftools)          SNP Extraction (bcftools)
       │
       ▼
QUAL Filtering (bcftools filter)
       │
       ▼
Variant Annotation (Ensembl VEP — GRCh37/GRCh38)
       │
       ▼
Functional Effect Prediction
(codon changes, amino acid changes, missense/silent classification)
```
